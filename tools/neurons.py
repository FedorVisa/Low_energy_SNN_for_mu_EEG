"""Spiking neuron modules and CUDA-backed neuron implementations."""
...
class CUPYPLIFNode(base.MemoryModule):
    def __init__(self, surrogate_function: Callable = surrogate.Sigmoid(), v_threshold: float = 1., v_reset: float = 0.,
        ...
        self.w = torch.nn.Parameter(torch.as_tensor(init_w).sigmoid())
    ...
    def multi_step_forward(self, x_seq: torch.Tensor):
        ...
        decay = self.w.to(device=x_seq.device, dtype=x_seq.dtype)
        # All tensors wil be regard as 2D or 1D. Thus, we use flatten
        spike_seq, v_seq = ParametricLIFNodeATGF.apply(x_seq.flatten(1), self.v.flatten(), self.v_threshold, self.v_reset, decay, forward_kernel, backward_kernel)
        spike_seq = spike_seq.view(x_seq.shape)
        self.v = v_seq[-1].view(x_seq.shape[1:])
        return spike_seq
