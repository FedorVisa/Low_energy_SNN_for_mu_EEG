"""Configuration helpers for auto-generated CUDA neuron kernels."""
...
def _build_cuda_compiler_options():
    options = ['-use_fast_math']
    try:
        import cupy
        if hasattr(cupy, 'get_include'):
            cupy_include = cupy.get_include()
        else:
            cupy_include = os.path.join(os.path.dirname(cupy.__file__), '_core', 'include')
        options.append(f'--include-path={cupy_include}')

        cuda_header_roots = sorted(glob(os.path.join(cupy_include, 'cupy', '_cuda', 'cuda-*')))
        if cuda_header_roots:
            options.append(f'--include-path={cuda_header_roots[-1]}')
    except Exception:
        pass
    return tuple(options)


cuda_compiler_options = _build_cuda_compiler_options()
'''
